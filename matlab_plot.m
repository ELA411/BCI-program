...
while ishandle(fig)
    ...
    if length(splitData) >= 5 % If the split was successful or, the package is not corrupt
        ...
        if ~isnan(adc1) && ~isnan(adc2) && ~isnan(packageid) && ~isnan(label) && ~isnan(timestamp)
            ...
            emgDataBuffer = [emgDataBuffer; adc1, adc2]; % Store values for SNR; SINAD, and THD calculation
            ...
            if lastPackageId ~= -1 && mod(packageid - lastPackageId - 1, 1000) ~= 0
                ...
                addpoints(h1, timeNow, adc1); % Add normal data point
                addpoints(h2, timeNow, adc2);
                fprintf(fileID,'%d %d %d %d %d\n',int32(adc1), int32(adc2), int32(label), int32(packageid), int32(timestamp));
